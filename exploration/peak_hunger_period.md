...
## Identifying peak lean season per country

Before running this notebook, make sure to create a `.env` file and set the `DSCI_AZ_BLOB_DEV_SAS` environment variable.
...
# Plot heatmap of aggregated data per country
...
Per country, we might call the peak lean season as the longest period between which the z-score is greater than the defined threshold. There will be some countries where this works, and some where it might not. See the plot below for an example.
...
create_heatmap(
    ...
    output_filename='../plots/heatmap_report_count.png',
    vmin=0,
    vmax=15,
    center=7.5,
    annot=True
